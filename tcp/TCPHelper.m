...
-(void)receiveDataBackground;
{
    ...
    dispatch_source_t readSource=dispatch_source_create(DISPATCH_SOURCE_TYPE_READ, sockfd, 0, socketQueue);
    
    dispatch_source_set_event_handler(readSource, ^{
        NSAutoreleasePool *eventPool = [[NSAutoreleasePool alloc] init];
        
        
        unsigned long numPendingReadings = dispatch_source_get_data(readSource);
        if (numPendingReadings > 0) {
           // printf("--%lu",numPendingReadings);
...
-(void) sendDataAndWaitDone:(NSData *)data;
{
    if (![self isConnected])
	{
